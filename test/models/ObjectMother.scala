...
object ObjectMother {
  ...
  def adminUser(): User = User(None, "a1", "af1","al1", "", true, "emaila1", "pwa1", true, true, true, true, true, 3, 0, g("emaila1")._1, g("emaila1")._2, None, false, DomainHelper.filterSettings())   
  
  def dummyUsers(): Seq[User] = (1 to 3).map{ nr => 
	  val email =  s"f${nr}@betting.at"
	  val (g,t) = DomainHelper.randomGravatarUrl(email)
	  User(None, s"u$nr", s"f$nr", s"l$nr", "", true, email, s"$nr", true, true, true, true, true, 4 , 2, g, t, None, false, DomainHelper.filterSettings())
  }
  
  val dummyTeams: Seq[Team] = (1 to 6).map{nr => Team(None, s"t$nr", s"t$nr", s"t$nr")}
   
  val dummyPlayers: Seq[Player] = (1 to 6).map{ nr => Player(None, s"playername$nr", "role", s"clubname$nr", 0, DBImage("",""), s"playername$nr")}
  ...
  def specialTemplates(stype: String, start: OffsetDateTime): Seq[SpecialBetT] = {
	  ...
	  (1 to 6).map{ i =>  
		  if(i < 4){ //first 3 -> grouped bets 
		     SpecialBetT(None, s"$nm $i", s"$nm $i", 1 , start, stype+"1" , stype, "", i )
	      }else{
		      SpecialBetT(None, s"$nm $i", s"$nm $i", i , start, stype+"1" , stype, "", i )
		  }
	   }.toList
   }
