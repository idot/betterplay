...
class TestData(betterDb: BetterDb, environment: Environment) extends InitialData{
  ...
  def teamsGames(levelId: Long): (Set[Team], Seq[(String,String,Game)]) = {
      ...
      val lines = TestData.toLines("GAMES2014.txt", environment)    
...
//	     betterDb.specialbetsuser.filter(s => s.spId === 3l).map(_.prediction).update("Argentinia")  
 // }
  ...
  def insert(debug: Boolean): Unit = { 
    val ls = TestData.levels(environment)
    val us = users(debug)
    val ps = players()
    val sp = TestData.specialBets(new DateTime(2014, 6, 12, 22, 0))
